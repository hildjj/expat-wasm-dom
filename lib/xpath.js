...
const grammar = require('./xpathPattern3')
...
class XPathSyntaxError extends Error {
  ...
  [util.inspect.custom] (depth, options) {
    ...
    let ret = this.pattern.slice(0, this.location.start.offset)
...
class XPath {
  ...
  _eval (context, [name, ...args]) {
    // console.log('_eval', name, util.inspect(context, {depth: 1, colors: true}))
