 ...
 * @prop {boolean} [compressed=false]
 * @prop {number} [indent=0]
 * @prop {string} [indentText]
 * @prop {string} [newline]
 * @prop {boolean} [pretty=false]
 * @prop {string} [quote="'"]
 * @prop {boolean} [removeComments=false]
...
const TOSTRING_OPTS = {
  ...
  /** @type {LineWrap | undefined} */
  wrap: undefined,
};

// Make HTML state less obvious to people looking at
// dom objects.  Most of the overhead is in statics, and there is an HTML
// bool on each node used for stringifying.
const HTML = Symbol('HTML');
const SET_HTML = Symbol('SET_HTML');
...
export class Node {
   ...
   */
  static [SET_HTML](node, html) {
    const ret = node[HTML] !== html;
    node[HTML] = html;
    return ret;
  }
  ...
  toString(options = TOSTRING_OPTS) {
    if (!options.collator) {
      options.collator = new Intl.Collator();
    }
    if (!options.wrap) {
   ...
   *
   * @param {string} [_newText] New text to insert
...
export class ParentNode extends Node {
   ...
   */
  static [SET_HTML](node, html) {
    if (Node[SET_HTML](node, html)) {
      // @ts-ignore
      node.children.forEach(c => c.constructor[SET_HTML](c, html));
      return true;
    }
    return false;
  }
  ...
  add(node) {
    ...
    node.parent = this;
    // @ts-ignore
    node.constructor[SET_HTML](node, this[HTML]);
...
export class Element extends ParentNode {
  ...
  constructor(name, attribs = [], ns = []) {
    ...
    this.att = Array.isArray(attribs) ?
      attribs.map(([n, v]) => new Attribute(n, v)) :
      this.att = Object.entries(attribs)
        .map(([k, v]) => new Attribute({local: k}, v));
   ...
   */
  static [SET_HTML](node, html) {
    if (ParentNode[SET_HTML](node, html)) {
      // @ts-ignore
      node.ns.forEach(n => n.constructor[SET_HTML](n, html));
      // @ts-ignore
      node.att.forEach(a => a.constructor[SET_HTML](a, html));
      return true;
    }
    return false;
  }
  ...
  toString(options = TOSTRING_OPTS) {
    ...
    if (!options.compressed && options.pretty) {
    ...
    if (options.pretty) {
      const ns = [...this.ns].sort((a, b) => Namespace.compare(
        a, b, /** @type {Intl.Collator} */ (options.collator)
      ));
      const att = [...this.att].sort((a, b) => Attribute.compare(
        a, b, /** @type {Intl.Collator} */ (options.collator)
      ));
