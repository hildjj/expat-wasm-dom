...
export class Node {
   ...
   * @param {bool} [options.colors=false] Use colors for output
   ...
   * @param {object} [options={}] Other inspection options
   * @param {boolean} [options.colors=false] If true, the output is styled
   ...
   *   if none is provided.
   * @returns {array<Node>} The matching nodes
   */
  get(pattern, context) {
   ...
   * @param {Node} [context=this] The context node.  Uses this node as context
   *   if none is provided.
   * @returns {Node|null} The first match if one exists
   */
...
export class ParentNode extends Node {
  ...
  *elements(local, ns) {
   ...
   * @param {object} [options] How to convert to string
   * @param {number} [options.depth=Infinity] Number of layers of children to
   *   output, use `Infinity` or null for all.
   * @param {bool} [options.colors=false] Use colors for output
   * @param {string} [options.separator=''] Separator between each of the
   *   children
   * @returns {string} The node, converted to a string
   */
  toString(options = TOSTRING_OPTS) {
...
export class Element extends ParentNode {
   ...
   * @param {Array<Array<string|Pieces, string>>} [attribs=[]] List of
   *   name/value pairs for attributes.
   * @param {Array<Array<string, string>>} [ns=[]] List of prefix/URI pairs
   ...
   * @returns {Attribute|null} Attribute, if one that matches is found
   */
  attribute(local, ns) {
  ...
  removeAttribute(name) {
    ...
    this.att = this.att.filter(att => {
      if (att.name.local !== name.local) {
        return true
      }
      if (name.ns && (att.name.ns !== name.ns)) {
        return true
      }
   ...
   *   in any namespace.
   * @returns {Element|null} The first matching element, if one is found.
   */
  element(local, ns) {
    return this.children.find(e => (e instanceof Element) &&
      (!ns || (ns === e.name.ns)) &&
      (local === e.name.local))
  }
   ...
   *
   * @param {string} [newText=null] If given, set the text of the element.
   *   Otherwise, gets all descendant text, concatenated together.
   * @returns {Text|string} Text on set, string on get.
   */
  text(newText = null) {
    if (newText == null) {
      return super.text()
...
export class Text extends Node {
   ...
   *
   * @param {string} [newText=null] If given, set the text of the node.
   *   Otherwise, gets the contained text.
   * @returns {null|string} null on set, string on get.
   */
  text(newText = null) {
    if (newText == null) {
      return this.txt
